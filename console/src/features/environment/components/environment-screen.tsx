...
import type { EnvironmentResource, MachineSummary } from "../../../common/api/types";
...
function formatMachineDisplayLabel(machine: MachineSummary | undefined, fallbackId: string) {
  if (!machine) {
    return fallbackId;
  }
  ...
  const statusLabel =
    machine.status === "reconnecting"
      ...
      : machine.status === "unknown"
        ? "未知"
        : "";
  const baseName = machine.name || machine.id || fallbackId;
  return statusLabel ? `${baseName} (${statusLabel})` : baseName;
}

function formatAgentOptionLabel(agent: Record<string, unknown>) {
  const name =
  ...
  const suffix =
    ...
    (typeof agent.agentType === "string" && agent.agentType.trim()) ||
    "";
...
export function EnvironmentScreen({ enabled }: EnvironmentScreenProps) {
  const vm = useEnvironmentPage({ enabled });
  const dialogContainer =
    typeof document === "undefined" ? undefined : document.querySelector("main") ?? undefined;
  const onlineMachines = useMemo(
    () =>
      vm.machines.filter((machine) => machine.status === "online" && (machine.agents?.length ?? 0) > 0),
    [vm.machines],
  );
  const machineById = useMemo(
    () => new Map(vm.machines.map((machine) => [machine.id, machine])),
    [vm.machines],
  );

  const handleMachineSelection = (type: ResourceType, machineId: string) => {
    const machine = machineById.get(machineId);
    const defaultAgentId = machine?.agents?.[0]?.agentId ?? "";

    if (type === "skill") {
      vm.setSkillForm((current) =>
        current ? { ...current, machineId, agentId: defaultAgentId } : current,
      );
      return;
    }
  ...
  const renderMachineAgent = (resource: EnvironmentResource) => {
    ...
    const machineLabel = formatMachineDisplayLabel(machine, resource.machineId);
    const agent =
      machine?.agents?.find((item) => item.agentId === resource.agentId) ?? machine?.agents?.[0] ?? null;
    ...
    return (
      <div className="flex items-center gap-2 text-[10px] text-zinc-600">
        ...
        <span>
          {agent
            ? formatAgentOptionLabel(agent as unknown as Record<string, unknown>)
            : resource.agentId ?? "默认 Agent"}
        </span>
      </div>
    );
  };
  ...
  const renderAgentSelect = (
    ...
    if (!machine || (machine.agents?.length ?? 0) === 0) {
      return null;
    }

    return (
      <div>
        <label className="block text-xs text-zinc-400 mb-2" htmlFor={inputId}>
          {label}
        </label>
        <select
          id={inputId}
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-950 text-zinc-100">
      ...
      <Dialog.Root
        modal={false}
        open={Boolean(vm.skillForm)}
        onOpenChange={(open) => !open && vm.setSkillFormClosed()}
      >
            ...
            {vm.skillForm ? (
              <form className="space-y-4" onSubmit={(event) => void vm.handleSkillSubmit(event)}>
                ...
                {renderAgentSelect(
                  "目标 Agent",
                  "skill-agent",
                  machineById.get(vm.skillForm.machineId),
                  vm.skillForm.agentId,
                  (value) =>
                    vm.setSkillForm((current) => (current ? { ...current, agentId: value } : current)),
                )}
      ...
      <Dialog.Root
        modal={false}
        open={Boolean(vm.mcpForm)}
        onOpenChange={(open) => !open && vm.setMcpFormClosed()}
      >
        <Dialog.Portal container={dialogContainer}>
          <Dialog.Overlay className="fixed inset-0 bg-black/50 z-50" />
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl w-full max-w-md z-50 p-6">
            ...
            {vm.mcpForm ? (
              <form className="space-y-4" onSubmit={(event) => void vm.handleMCPSubmit(event)}>
                ...
                {renderAgentSelect(
                  "目标 Agent",
                  "mcp-agent",
                  machineById.get(vm.mcpForm.machineId),
                  vm.mcpForm.agentId,
                  (value) =>
                    vm.setMcpForm((current) => (current ? { ...current, agentId: value } : current)),
                )}
      ...
      <Dialog.Root
        modal={false}
        open={Boolean(vm.pluginForm)}
        onOpenChange={(open) => !open && vm.setPluginFormClosed()}
      >
        <Dialog.Portal container={dialogContainer}>
          <Dialog.Overlay className="fixed inset-0 bg-black/50 z-50" />
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl w-full max-w-md z-50 p-6">
            ...
            {vm.pluginForm ? (
              <form
                className="space-y-4"
                onSubmit={(event) => void vm.handlePluginInstallSubmit(event)}
              >
                <div>
                ...
                {renderAgentSelect(
                  "目标 Agent",
                  "plugin-agent",
                  machineById.get(vm.pluginForm.machineId),
                  vm.pluginForm.agentId,
                  (value) =>
                    vm.setPluginForm((current) =>
                      current ? { ...current, agentId: value } : current,
                    ),
                )}
                ...
                <div>
                  <label className="block text-xs text-zinc-400 mb-2" htmlFor="plugin-path">
                    Marketplace path
                  </label>
                  <input
                    id="plugin-path"
