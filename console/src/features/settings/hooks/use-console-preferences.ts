...
export function useConsolePreferences(options?: UseConsolePreferencesOptions) {
  ...
  useEffect(() => {
    if (!enabled || store.state.isLoading) {
      return;
    }
  ...
  }, [connectionIdentity, enabled, loadPreferences]);

  const savePreferences = useCallback(async (next: ConsolePreferences) => {
  ...
  }, [snapshot.hasAttempted, snapshot.loadError, snapshot.preferences]);
