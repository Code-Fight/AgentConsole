        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          {machine.sessions.length > 0 ? (
            <span className="text-[10px] text-zinc-500 bg-zinc-800 px-1.5 py-0.5 rounded">
              {machine.sessions.filter((s) => s.status === "active").length > 0
                ? `${machine.sessions.filter((s) => s.status === "active").length} 活跃`
...
function NewThreadDialog({
  ...
  const handleCreate = () => {
    if (!selectedMachineId || !selectedAgentId || !title.trim()) return;
...
export default function MachinePanel({
  ...
  const totalActive = machines.reduce(
    (sum, m) => sum + m.sessions.filter((s) => s.status === "active").length,
    0,
  );
